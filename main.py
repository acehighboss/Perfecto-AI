...
def main():
    ...
    with col1:
        ...
        if st.button("💬 대화 초기화", help="채팅 기록을 삭제합니다", use_container_width=True):
            st.session_state.messages = []
            st.rerun()
        
        if st.button("🗑️ 전체 초기화", help="모든 데이터를 삭제합니다", use_container_width=True):
            st.session_state.messages = []
            if st.session_state.rag_pipeline:
    ...
    with col2:
        ...
        placeholder_text = "질문을 입력하세요..." if not input_disabled else "먼저 문서를 업로드하고 분석을 완료해주세요."
